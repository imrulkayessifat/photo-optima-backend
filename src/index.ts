    ...
    connection.createChannel(function (error1, channel) {
        ...
        channel.consume(queue, async function (msg: { content: { toString: () => any; }; }) {
            ...
            try {
                ...
                const response = await fetch(`${process.env.MQSERVER}/image/restore-upload`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ uid, productid, url, store_name })
                });
                ...
                if (!response.ok) {
                    console.error(`Failed to restore image: ${response.statusText}`);
                } else {
                    const responseData = await response.json();
                    console.log(`Image restored successfully: ${responseData}`);
                }
            } catch (error) {
                console.error('Error processing message', error);
            }
    ...
    connection.createChannel(function (error1, channel) {
        ...
        channel.consume(queue, async function (msg: { content: { toString: () => any; }; }) {
            try {
                ...
                const imageData = await db.image.findFirst({ where: { uid: parseInt(uid) } });
                if (!imageData) {
                    console.error(`Image with UID ${uid} not found`);
                    return;
                }
                ...
                if (productid !== '1') {
                    ...
                    const base64Image = Buffer.from(url, 'base64');
                    ...
                    if (getUploadcareImage.status === 200) {
                        await fetch(`https://api.uploadcare.com/files/${imageData.id}/storage/`, {
                            method: 'DELETE',
                            headers: {
                                'Authorization': `Uploadcare.Simple ${process.env.UPLOADCARE_PUBLIC_KEY}:${process.env.UPLOADCARE_SECRET_KEY}`
                            }
                        ...
                        console.log(`Image uploaded to Uploadcare: ${data}`);
                    } else {
                        console.error('Failed to delete image from Uploadcare');
                    }
                }
                ...
                if (existBackupImageFromCustomDB.status === 200) {
                    await fetch(`${process.env.MQSERVER}/backup/${uid}`, {
                        method: 'DELETE'
                    });
                }
            } catch (error) {
